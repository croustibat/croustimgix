...
function imagist(opts = {}) {
  ...
  const _allowedFits = ['cover', 'contain', 'fill', 'inside', 'outside']

  const _allowedPositions = ['top', 'bottom', 'left', 'right', 'center', 'entropy', 'attention']

  const _allowedInterpolations = ['nearest', 'cubic', 'mitchell', 'lanczos2', 'lanczos3']

  const _allowedFlips = ['h', 'v', 'both']
  ...
  async function _processImage(reader, mimeType, query = {}) {
    ...
    const methods = ['trim', 'rotate', 'flip', 'flop', 'sharpen', 'blur', 'negate', 'greyscale', 'tint', 'withMetadata']
